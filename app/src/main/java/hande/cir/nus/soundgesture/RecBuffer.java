...
public class RecBuffer implements Runnable {
	...
	private static final int BUFSIZE = 4096*2;//48000 * 2 * 10 / 1000; inputBlockSize*2
...
//			SystemClock.sleep(100);
			...
			byte[] buffer = new byte[BUFSIZE * 2]; //4096*4
			int read;
...
//			os.writeBytes("/system/bin/tinycap /sdcard/tmp.wav -D 1 -d 0 -c 2 -r 44100 -b 16\n"); //nexus 7
			os.writeBytes("/system/bin/tinycap /sdcard/tmp.wav -D 0 -d 1 -c 2 -r 48000 -b 16\n"); //galaxy nexus

            //"Usage: %s file.wav [-d device] [-c channels] [-r rate] [-b bits]\n"

			os.flush();
			SystemClock.sleep(100);

			read = buffer.length;//4096*4
			while (true) {
//				Log.d(LTAG, "try to read stdio tinycap" );
				...
				short[] outData = new short[read / 2]; //4096*2
