    ...
    private void readerRun() {
        ...
        try {
            ...
            while (running) {
                ...
                int space = inputBlockSize - inputBufferIndex; //initial 0
                if (readSize > space)
                    readSize = space;
                ...
                synchronized (buffer) {
                    int nread = audioInput.read(buffer, index, readSize); //read data into buffer
                    ...
                    if (end >= inputBlockSize) { //不断取样直到达到这个数目
                        inputBufferWhich = (inputBufferWhich + 1) % 2;//0,1 交替
                        inputBufferIndex = 0;
                        done = true;
                    } else
