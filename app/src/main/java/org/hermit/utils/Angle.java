...
public class Angle
{
	 ...
	 */
	public static org.hermit.utils.Angle fromDegrees(double degrees) {
		return new org.hermit.utils.Angle(toRadians(degrees));
	}
	 ...
	 * @return          The corresponding Angle.
	 */
	public static org.hermit.utils.Angle fromDegrees(int d, int m, double s) {
		boolean neg = d < 0 || m < 0 || s < 0;
		double df = ((abs(s) / 60.0 + abs(m)) / 60.0 + abs(d));
		return new org.hermit.utils.Angle(toRadians(neg ? -df : df));
	}
     ...
     * @return          The corresponding Angle.
	 */
	public static org.hermit.utils.Angle fromRightAscension(int rh, int rm, double rs) {
		boolean neg = rh < 0 || rm < 0 || rs < 0;
		double ra = ((abs(rs) / 60.0 + abs(rm)) / 60.0 + abs(rh)) * 15.0;
		return new org.hermit.utils.Angle(toRadians(neg ? -ra : ra));
	}
	 ...
	 */
	public org.hermit.utils.Angle add(double radians) {
		return new org.hermit.utils.Angle(angleR + radians);
	}
    ...
    public String formatDegMin() {
        return org.hermit.utils.Angle.formatDegMin(toDegrees(angleR)) + '°';
    }
    ...
    public String formatDegMinSec() {
        return org.hermit.utils.Angle.formatDegMinSec(toDegrees(angleR)) + '°';
    }
